...
const CourtOwnerSubscribePayment = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setIsLoading(false)
      navigate('/courtowner/subscription-success')
    }, 2000)
  }
  ...
  return (
    <div className="min-h-screen bg-dark-bg pb-6">
      <PageHeader title="Complete Payment" backPath="/courtowner/subscription" />
      ...
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <Card title="Payment Method" padding="p-6">
          ...
          <div className="space-y-3 mb-6">
            <label className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors ${
              formData.paymentMethod === 'gcash' ? 'border-red-600 bg-red-50' : 'border-dark-border hover:border-dark-border'
            }`}>
              <input
                type="radio"
                name="paymentMethod"
            ...
            <label className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors ${
              formData.paymentMethod === 'card' ? 'border-red-600 bg-red-50' : 'border-dark-border hover:border-dark-border'
            }`}>
              <input
                type="radio"
                name="paymentMethod"
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-dark-border">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-dark-bg-hover peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-red-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-dark-bg-tertiary after:border-dark-border after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-600"></div>
              </label>
            </div>
          </div>
        </Card>
        ...
        <Card padding="p-6">
          <label className="flex items-start space-x-3 cursor-pointer">
            <input
              ...
              onChange={handleChange}
              className="w-5 h-5 text-brand-primary border-dark-border rounded focus:ring-red-500 mt-0.5"
            />
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center space-x-2"
        >
          {isLoading ? (
            <>
...
export default CourtOwnerSubscribePayment
