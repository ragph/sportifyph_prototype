import { useNavigate } from 'react-router-dom'
import {
  IoCreate,
  IoShareSocial,
  IoFitness,
  IoStar,
  IoPeople,
  IoCash,
  IoCalendar,
  IoTrophy,
  IoMail,
  IoCall,
  IoLocationSharp,
  IoTime
} from 'react-icons/io5'

const CoachProfile = () => {
  const navigate = useNavigate()

  const profile = {
    name: 'Coach Mike Santos',
    username: '@coachmike',
    bio: 'Professional pickleball coach with 10+ years of experience. Specialized in advanced techniques, tournament preparation, and player development. Passionate about helping athletes master the game and reach their full potential.',
    rating: 4.9,
    reviewCount: 127,
    memberSince: 'January 2023',
    avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&h=200&fit=crop'
  }

  const stats = [
    { icon: IoCalendar, value: '450+', label: 'Total Sessions', color: 'green' },
    { icon: IoPeople, value: '45', label: 'Active Students', color: 'green' },
    { icon: IoCash, value: '₱225K', label: 'Total Earnings', color: 'green' },
    { icon: IoStar, value: '4.9', label: 'Rating', color: 'green' }
  ]

  const specializations = [
    { name: 'Pickleball Fundamentals', experience: '10+ years' },
    { name: 'Tournament Strategy', experience: '8 years' },
    { name: 'Advanced Shot Techniques', experience: '10+ years' }
  ]

  const certifications = [
    'USAPA Certified Pro',
    'PPR Certified Coach',
    'Level 4.5+ Player Rating',
    'Sports Science Diploma'
  ]

  const contactInfo = [
    { icon: IoMail, label: 'Email', value: 'mike.santos@email.com' },
    { icon: IoCall, label: 'Phone', value: '+63 912 345 6789' },
    { icon: IoLocationSharp, label: 'Location', value: 'Makati, Manila' },
    { icon: IoTime, label: 'Availability', value: 'Mon-Sat, 6 AM - 8 PM' }
  ]

  return (
    <div className="min-h-screen bg-dark-bg pb-20">
      <div className="max-w-md mx-auto p-4 space-y-6">
        {/* Profile Header */}
        <div className="bg-dark-bg-tertiary rounded-xl shadow-sm overflow-hidden">
          <div className="h-24 bg-gradient-to-r from-green-600 to-emerald-600"></div>
          <div className="relative px-4 pb-4">
            <img
              src={profile.avatar}
              alt={profile.name}
              className="w-24 h-24 rounded-full border-4 border-white -mt-12 mb-3 object-cover"
            />

            <div className="mb-4">
              <h1 className="text-2xl font-bold text-dark-text">{profile.name}</h1>
              <p className="text-sm text-brand-primary">{profile.username}</p>
              <div className="flex items-center space-x-1 mt-2">
                <IoStar className="text-brand-primary" />
                <span className="text-sm font-semibold text-dark-text">{profile.rating}</span>
                <span className="text-xs text-dark-text-muted">({profile.reviewCount} reviews)</span>
              </div>
            </div>

            <div className="flex space-x-3 mb-4">
              <button
                onClick={() => navigate('/coach/edit-profile')}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-lg py-3 px-4 font-semibold transition-colors flex items-center justify-center space-x-2"
              >
                <IoCreate />
                <span>Edit Profile</span>
              </button>
              <button
                onClick={() => {}}
                className="flex-1 bg-dark-bg-secondary hover:bg-dark-bg-hover text-dark-text rounded-lg py-3 px-4 font-semibold transition-colors flex items-center justify-center space-x-2"
              >
                <IoShareSocial />
                <span>Share</span>
              </button>
            </div>
          </div>
        </div>

        {/* About */}
        <div className="bg-dark-bg-tertiary rounded-xl p-4 shadow-sm">
          <h2 className="text-lg font-bold text-dark-text mb-3">About</h2>
          <p className="text-sm text-dark-text-secondary">{profile.bio}</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 gap-4">
          {stats.map((stat, index) => {
            const IconComponent = stat.icon
            return (
              <div key={index} className="bg-dark-bg-tertiary rounded-xl p-4 shadow-sm border border-dark-border">
                <IconComponent className={`text-2xl text-${stat.color}-600 mb-2`} />
                <div className="text-2xl font-bold text-dark-text">{stat.value}</div>
                <div className="text-xs text-dark-text-secondary">{stat.label}</div>
              </div>
            )
          })}
        </div>

        {/* Specializations */}
        <div className="bg-dark-bg-tertiary rounded-xl p-4 shadow-sm">
          <h2 className="text-lg font-bold text-dark-text mb-4">Specializations</h2>
          <div className="space-y-3">
            {specializations.map((spec, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-dark-bg rounded-lg">
                <div className="flex items-center space-x-3">
                  <IoFitness className="text-xl text-brand-primary" />
                  <span className="text-sm font-medium text-dark-text">{spec.name}</span>
                </div>
                <span className="text-sm font-bold text-brand-primary">{spec.experience}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Certifications & Achievements */}
        <div className="bg-dark-bg-tertiary rounded-xl p-4 shadow-sm">
          <h2 className="text-lg font-bold text-dark-text mb-4">Certifications & Achievements</h2>
          <div className="grid grid-cols-2 gap-2">
            {certifications.map((cert, index) => (
              <div key={index} className="flex items-center space-x-2 p-2 bg-brand-primary/10 rounded-lg">
                <div className="w-2 h-2 rounded-full bg-green-600"></div>
                <span className="text-sm text-dark-text">{cert}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Contact Information */}
        <div className="bg-dark-bg-tertiary rounded-xl p-4 shadow-sm">
          <h2 className="text-lg font-bold text-dark-text mb-4">Contact Information</h2>
          <div className="space-y-3">
            {contactInfo.map((info, index) => {
              const IconComponent = info.icon
              return (
                <div key={index} className="flex items-center space-x-3">
                  <IconComponent className="text-xl text-dark-text-secondary" />
                  <div className="flex-1">
                    <p className="text-xs text-dark-text-muted">{info.label}</p>
                    <p className="text-sm font-medium text-dark-text">{info.value}</p>
                  </div>
                </div>
              )
            })}
          </div>
        </div>

        {/* Member Since */}
        <div className="text-center py-4">
          <p className="text-xs text-dark-text-muted">Member since {profile.memberSince}</p>
        </div>
      </div>
    </div>
  )
}

export default CoachProfile
